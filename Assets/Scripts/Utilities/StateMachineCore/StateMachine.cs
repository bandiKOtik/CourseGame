...
{
    public abstract class StateMachine<TState> : State, IUpdateableState, IDisposable where TState : class, IState
    {
        ...
        public void AddTransition(TState fromState, TState toState, ICondition condition)
        {
            ...
            from.AddTransition(new(to, condition));
        }
        ...
        public void Dispose()
        {
            ...
            foreach (var node in _states)
                if (node.State is IDisposable disposableState)
